...
const DisplaysKpiService = {
   ...
   */
  async buscarDados() {
    const resposta = await fetch("/api/alertas/dados-dashboard", {
      method: "GET",
    });
...
const DisplaysKpiController = {
  ...
  _atualizarStatusTabela(dados) {
    ...
    for (const [idDisplay, statusGeral] of Object.entries(statusPorDisplay)) {
      ...
      const celulasStatus = linha.querySelectorAll(
        ".status-container .g-status",
      );
...
(function aguardarTabela() {
  ...
  function verificar() {
    ...
    const linhas = document.querySelectorAll(
      "#lista_displays tr:not(#primeiro_item)",
    );
    ...
    if (tentativas < TENTATIVAS_MAX) {
      ...
      console.warn(
        "[DisplaysKpiController] Tabela sem linhas após espera, tentando mesmo assim.",
      );
      DisplaysKpiController.init();
    }
  }
