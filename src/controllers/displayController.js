...
function cadastrarDisplay(req, res) {
  ...
  if (nome == undefined) {
    ...
    displayModel
      .cadastrarDisplay(fkEmpresa, nome, id, so, ip, zona, mac)
...
function cadastrarComp(req, res) {
  ...
  if (Componentes == undefined) {
    ...
    displayModel
      .cadastrarComp(Componentes, fkEmpresa, fkServidor)
      .then(function (resultado) {
        res.json(resultado);
      })
      .catch(function (erro) {
...
// Busca da barra de busca da tela de displays
function buscarDisplay(req, res) {
  let sentenca = req.body.sentenca;
  let fkEmpresa = req.body.fkEmpresa;

  if (sentenca === undefined) {
    return res
      .status(400)
      .json({ mensagem: "A sentença de busca não foi fornecida." });
  }

  if (fkEmpresa === undefined) {
    return res
      .status(400)
      .json({ mensagem: "A fkEmpresa não foi fornecida." });
  }

  displayModel
    .buscarDisplay(sentenca, fkEmpresa)
    .then((result) => {
      if (result && result.length > 0) {
        ...
        return res
          .status(404)
          .json({ mensagem: "Não foram encontrados dados" });
      }
    })
    .catch((erro) => {
      console.error("Erro detalhado na consulta SQL:", erro);

      return res.status(500).json({
        mensagem: "Ocorreu um erro interno no servidor ao processar a busca.",
